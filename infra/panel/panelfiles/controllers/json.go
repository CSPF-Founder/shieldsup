...
func (app *App) SendJSONError(w http.ResponseWriter, message string, statusCodeOption ...int) {
	...
	jsonOutput := map[string]any{"error": message}
	app.JSONResponse(w, jsonOutput, statusCode)
}

// SendJSONSuccess displays JSON success messages and exits the program.
func (app *App) SendJSONSuccess(w http.ResponseWriter, message any, statusCodeOption ...int) {
	if message == "" {
		message = "Error Occurred"
	}
	...
	case string:
		jsonOutput = map[string]any{"success": message}
	case map[string]any:
		jsonOutput = v
	default:
		jsonOutput = map[string]any{"success": message}
