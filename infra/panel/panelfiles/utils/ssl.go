...
func prepareCert(priv *ecdsa.PrivateKey) ([]byte, error) {
	notBefore := time.Now()
	...
	if err != nil {
		return nil, fmt.Errorf("failed to generate a random serial number: %w", err)
	}

	template := x509.Certificate{
		SerialNumber: serialNumber,
		Subject: pkix.Name{
			Organization: []string{"Shieldsup"},
		},
		NotBefore: notBefore,
		NotAfter:  notAfter,
...
func CheckAndCreateSSL(cp string, kp string) error {
	...
	derBytes, err := prepareCert(priv)
	if err != nil {
		return fmt.Errorf("tls certificate generation: failed to prepare certificate: %w", err)
	}
