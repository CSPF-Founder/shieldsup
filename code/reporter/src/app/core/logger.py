...
class Logger:
    ...
    * Logger class
    ...
    def __init__(self, logger_config: LoggerConfig):
        ...
        if not self.logger_config.log_format:
            self.log_formater = logging.Formatter(
                "%(levelname)s-%(asctime)s :%(message)s\n---------", "%Y-%m-%d %H:%M:%S"
            )
        else:
            self.log_formater = logging.Formatter(
                self.logger_config.log_format, "%Y-%m-%d %H:%M:%S"
            )
    ...
    def get_log_object(self):
        ...
        * get logging object
        ...
        log_handler.setFormatter(self.log_formater)
...
class AppLogger(LoggerWithInfoAndErrorFiles):
    ...
    * App logger class
    ...
    def __init__(
        ...
        super().__init__(info_log_path, exception_log_path, log_name, max_verbose_level)
